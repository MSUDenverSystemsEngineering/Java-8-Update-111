...
Try {
	...
	[string]$appVersion = '8u111'
	[string]$appArch = 'x86'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.1.0'
	[string]$appScriptDate = '04/04/2017'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		## Show Welcome Message, close browsers if required, verify there is enough disk space to complete the install, and persist the prompt
		Show-InstallationWelcome -CloseApps 'iexplore,firefox' -CheckDiskSpace -PersistPrompt
		...
		$exitCode = Execute-Process -Path "jre-$appVersion-windows-i586.exe" -Parameters "/s STATIC=Disable AUTO_UPDATE=Enable WEB_JAVA=Enable WEB_ANALYTICS=Disable EULA=Disable REBOOT=Disable SPONSORS=Disable" -Windows "Hidden" -WaitForMsiExec -PassThru
		...
		$exitCode = Execute-MSI -Action 'Uninstall' -Path '{4A03706F-666A-4037-7777-5F2748764D10}'
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'iexplore,firefox' -CloseAppsCountdown 60
		...
		$exitCode = Execute-MSI -Action 'Uninstall' -Path '{26A24AE4-039D-4CA4-87B4-2F32180111F0}' -PassThru
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBXH57DrQE2VSGN
# +nXQZ5J4IsNZKXX2JJljceGptSRO7qCCD4cwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# gAAwGQYJKoZIhvcNAQkDMQwGCisGAQQBgjcCAQQwLwYJKoZIhvcNAQkEMSIEIBPb
# h8/CkCqD92TSC8p1BzOdpaas56wz5Nh87f/5tIa9MA0GCSqGSIb3DQEBAQUABIIB
# AFJ6vbQeRYZNcRZLxYrRbfJNF7i9PTSd/zCYMbfx4fLHcpCQSUs8m3Y+BILDDsAB
# Xb2lXrCmRFgli/WCkXE8pmLUwMna4VIMDrAy/OfQ+DKYDwl/eSAk/JM9+4LOsgYZ
# WJGItl+NhiqFadtGK/iz6cBs5gsBgETPRsjxGJVxIG7K/0uJe2w5Db50rGnqTXN5
# GrLpsWAj21K0kz/jB3QSI3qzXHT2DF8NgW3z99k7DUTUc+FWJ0CwTvulvvS7UeWZ
# 8gXE/B7PiSygGvJY8GmmrBRtbeVmoFbjhSBK0JodRzBWCNimAFIPuyt/DRzXr305
# o5OX/DL6flSeJXS+n9nZrXehggKiMIICngYJKoZIhvcNAQkGMYICjzCCAosCAQEw
...
# ATAcBgkqhkiG9w0BCQUxDxcNMTcwNDA0MTczOTM3WjAjBgkqhkiG9w0BCQQxFgQU
# qtC7mdkusOo7ytVOjEoTMARdJhcwgZ0GCyqGSIb3DQEJEAIMMYGNMIGKMIGHMIGE
...
# AQUABIIBAKOcPkHQPutyqiyQl1a64BCQPtQco8e/tiMfoQ1JWAKRQeFs3XtItceC
# 1x9JBoqQTh+kQN8KT9yztc3Q9fG1Lmxdkv7y+h7z0GjN9F8yHEKhE5yNtpb2pATS
# VtnDsxX0CU+32NBu+DNZD+AZw3JwpyjKheeGHjTLiMZpgOafXaOvJvcr2yXOxrvt
# JBlQ3ezwiAMv0BPhxtL5lz92qmJXtOjF4ctC8cztEgOHJJrmqIbtAfdefp6BvRhg
# OeyFZBgigFbltZG7kaIWMbytQqQ0pEXwvdK8iYdveI2Z6VCbr5J7/LYHW2xL/Nrs
# SCS0uWnJdiuOQhiOfif6O1+AcJoYDzM=
